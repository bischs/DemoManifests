...
argocd app get $app_name --server $ARGOCD_SERVER --auth-token "$ARGOCD_TOKEN"
if [ $? -ne 0 ]
then
...
argocd app set $app_name --parameter image.tag=$app_version --server "$ARGOCD_SERVER" --auth-token "$ARGOCD_TOKEN"
if [ $? -ne 0 ]
then
...
argocd app sync $app_name --server "$ARGOCD_SERVER" --auth-token "$ARGOCD_TOKEN"
if [ $? -ne 0 ]
then
...
argocd app wait $app_name --server "$ARGOCD_SERVER" --auth-token "$ARGOCD_TOKEN"
